...
#   curl -fsSL https://raw.githubusercontent.com/BancaKim/Bank_Knowledge_graph/master/deploy.sh | bash

set -euo pipefail

echo "=== 큽 Banking Bot 배포 시작 ==="
...
# 2. 소스 코드 클론/업데이트
REPO_DIR="$HOME/Bank_Knowledge_graph"
if [ -d "$REPO_DIR" ]; then
    echo ">> 기존 소스 업데이트..."
    cd "$REPO_DIR" && git pull origin master
else
    echo ">> 소스 클론..."
    git clone https://github.com/BancaKim/Bank_Knowledge_graph.git "$REPO_DIR"
    cd "$REPO_DIR"
fi
