...
> **[Live Demo](https://bancakim.github.io/Bank_Knowledge_graph/)** — 브라우저에서 바로 지식그래프를 탐색해보세요.

큽은행 공식 웹사이트([bank.example.com](https://bank.example.com))에 **공개된 금융상품 정보**를 수집하여 **Neo4j 지식그래프**로 구축한 프로젝트입니다. 162개 금융상품을 14개 엔티티 타입과 14개 관계 타입으로 구조화하고, D3.js 기반 인터랙티브 시각화와 GraphRAG 챗봇을 제공합니다.
...
큽은행 웹사이트에서 두 가지 방법으로 금융상품 데이터를 수집합니다.
...
channels: [인터넷, 모바일뱅킹, 고객센터]
eligibility_summary: 개인 및 개인사업자
source: 'PDF: 큽-Star-정기예금.pdf'
---
# 큽 정기예금
## 상품설명
## 금리
...
| **Channel** | 10 | 가입채널 | name (모바일뱅킹/인터넷/영업점) |
...
```mermaid
flowchart TD
    A["큽은행 웹사이트"] --> B["Playwright 스크래핑 / PDF 다운로드"]
...
```bash
# 1. 클론
git clone https://github.com/BancaKim/Bank_Knowledge_graph.git
cd Bank_Knowledge_graph
...
- 모든 금융상품 정보는 [큽은행 공식 웹사이트](https://bank.example.com)에 **일반에 공개된 정보**를 기반으로 수집되었으며, 비공개 정보나 내부 데이터는 포함되어 있지 않습니다.
- 수집된 정보는 **사실적 데이터**(상품명, 금리, 가입 조건 등)로서 지식그래프 형태로 구조화·변환한 것이며, 큽은행의 원본 콘텐츠를 그대로 재배포하는 것이 아닙니다.
- 본 프로젝트의 금융상품 정보는 **수집 시점 기준**이며, 실제 상품 조건과 다를 수 있습니다. 정확한 정보는 반드시 [큽은행 공식 웹사이트](https://bank.example.com)에서 확인하시기 바랍니다.
- 본 프로젝트는 큽은행과 어떠한 제휴·협약 관계에 있지 않습니다.
